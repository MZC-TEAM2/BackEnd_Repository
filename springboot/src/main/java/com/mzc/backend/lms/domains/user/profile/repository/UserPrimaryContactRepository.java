 ...
 */
@Repository
public interface UserPrimaryContactRepository extends JpaRepository<UserPrimaryContact, String> {

    /**
     * 사용자 ID로 주 연락처 조회
     */
    Optional<UserPrimaryContact> findByUserId(String userId);

    /**
     * 여러 사용자의 주 연락처 일괄 조회
     */
    @Query("SELECT upc FROM UserPrimaryContact upc WHERE upc.userId IN :userIds")
    List<UserPrimaryContact> findByUserIds(@Param("userIds") List<String> userIds);
